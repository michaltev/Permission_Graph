...
class ResourceNode(Node):
    def __init__(self, p_id, p_asset_type=None):
        if p_asset_type:
...
class IdentityNode(Node):
    def __init__(self, p_id):  # task6 > p_sub_type):
...
class Edge:
    def __init__(self, p_from_node, p_to_node, p_type):
...
class ParentEdge(Edge):
    def __init__(self, p_from_node, p_to_node):
        super().__init__(p_from_node, p_to_node, "parent of")
    pass


class RoleEdge(Edge):
    def __init__(self, p_from_node, p_to_node, p_role_type):
    ...
    def create_graph(self):
        with jsonlines.open('data_file.json') as reader:
            for line in reader:
                ...
                node_id = generate_id(line["name"])
                asset_type = generate_asset_type(line["asset_type"])
